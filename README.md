...
Linker는 학생의 오답을 지식 그래프 위에서 역추적해 진짜 막힌 개념을 찾아주고, AI 퀴즈와 교수 피드백으로 즉시 메꿔줍니다.

2026 KIT 바이브코딩 공모전 출품작 · 교육 AI 솔루션 부문

[데모](#데모) · [핵심 기능](#핵심-기능) · [Multi-Agent Harness](#multi-agent-harness--동작-원리) · [기술 스택](#기술-스택) · [AI 리포트](docs/AI_REPORT.md)
...
| 단계 | 행동 | 확인 포인트 |
|:---:|---|---|
| 1 | 사이트 접속 → **학생**으로 로그인 (아무 이메일/비밀번호) | 5단계 온보딩 슬라이드 |
| 2 | 온보딩 완료 → 튜토리얼 그래프 클릭 → 학습 페이지 진입 | 지식 그래프 캔버스 + 좌측 사이드바 |
| 3 | 좌측 "명확한 오답" 데모 프리셋 클릭 → **결손 개념 분석** | Multi-Agent SSE 실시간 스트리밍 (3단계 진행) |
| 4 | 결손 노드(빨강 pulse) 클릭 → 퀴즈 풀기 → 학습 완료 | 에이전트 트레이스 + 퀴즈 게이트 + 결손 전파 시각화 |
| 5 | **새 탭** 열기 → 교수로 로그인 → "예시 데이터로 둘러보기" 클릭 | 교수 대시보드 + 5개 학급 |
| 6 | 선형대수 1반 진입 → 학생 탭에서 분석 한 번 더 실행 | 교수 탭의 약점 분포가 실시간 +1 갱신 (양방향 루프) |

> 추가 체험: 교수 탭에서 노드 클릭 → 피드백 작성 → 학생 탭 우상단 종(알림) 확인 / 챗봇 "노드 추천" 모드에서 이미지 첨부 분석
...
- 학생도 자기 노드에 메모 가능
- 캔버스 우측에 교수 피드백 패널 — 노드별 대화 스레드 형태로 표시
...
### 8. 강의 텍스트 → 그래프 자동 생성 (3단계 플로우)
Claude Sonnet 4.6이 강의 노트를 DAG JSON으로 변환. 입력이 100자 미만이면 Haiku가 추가 질문(최대 3회)으로 컨텍스트를 보강한 뒤 Sonnet 1회 호출로 품질 확보. 교수 학급 생성 시 입력 → AI 생성 → 프리뷰(노드 리스트 확인) → 수정/재생성/배포 3단계 플로우 지원.

### 9. 이미지 기반 오답 분석 + 드래그 앤 드롭
수학 문제 사진을 드래그하거나 첨부하면 Claude Vision이 이미지를 분석해 Multi-Agent Harness로 결손 추적. 텍스트 입력 없이 이미지만으로도 분석 가능. 챗봇에서도 이미지 첨부 지원.

### 10. 추천 학습 경로 시각화
mastered 노드에서 다음 학습할 수 있는 frontier 노드까지의 경로를 에메랄드 점선으로 그래프 위에 표시. "다음에 뭘 공부해야 하는지" 시각적으로 즉시 파악.

### 11. 노드 콘텐츠 편집 + 안전한 외부 링크
노드별 학습 노트, 강의 영상 URL (YouTube 자동 썸네일), 참고 자료 링크 추가/편집. 외부 링크 클릭 시 도메인 안전 검증 + 이동 확인 팝업.

### 12. 그래프 내보내기 (PNG) + 키보드 단축키 + AI 학습 리포트
- 캔버스 "내보내기" 버튼으로 2x 해상도 PNG 다운로드
- Ctrl+Enter 분석 시작, Esc 패널 닫기
- 학생 홈에서 "AI 학습 리포트" — 강점/결손/추천을 AI가 종합 분석 (스트리밍 생성)

### 13. 개인 학습 인사이트 + Frontier 추천
...
- 모달로 본문 확인, 개별 읽음 처리 (클릭 시) / 역할별 숨기기 / 전체 삭제

### 15. 다크/라이트 모드
홈 헤더에서 Sun/Moon 토글로 즉시 전환. FOUC 방지를 위해 `layout.tsx`에 인라인 스크립트로 초기 테마 적용. 온보딩/로그인/학습 페이지 등 모든 라우트가 양쪽 테마에 대응합니다.
...
| AI 퀴즈 자동 생성 | △ 수동 요청 | X | O 노드별 맞춤 퀴즈 즉석 생성 |
...
- [x] 멀티모달 AI 챗봇 (Q&A/퀴즈/노드 추천 3모드 + 이미지 첨부 + 추천 노드 캔버스 직접 추가)
- [x] 역할별 soft-delete (학생/교수 각자 삭제해도 상대방 데이터 유지)
- [x] 그래프 폐지 라이프사이클 (강좌 종료 → retire → 학생 알림)
- [x] 이미지 기반 오답 분석 (Claude Vision + 드래그 앤 드롭)
- [x] 추천 학습 경로 시각화 (에메랄드 점선 + frontier 노드 하이라이트)
- [x] 노드 콘텐츠 편집 (학습 노트 / 영상 URL / 참고 자료) + 외부 링크 안전 검증
- [x] 그래프 PNG 내보내기 + 키보드 단축키 (Ctrl+Enter, Esc)
- [x] AI 학습 종합 리포트 (스트리밍 생성)
- [x] 데모 모드 (mock 5개 학급) ↔ 실사용 학급 통합 뷰 + 데모 리셋
- [x] 다크/라이트 모드 (FOUC 방지 포함)
